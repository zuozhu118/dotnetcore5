...
define([], function () {
  ...
  var Drawing = useSVG
    ...
    : (function () {
        ...
        Drawing.prototype.addIcon = async function (icon) {
          ...
          const ratio = this._htOption.iconRatio;
          const radius = 10;
  ...
  QRCode.prototype.makeCode = async function (sText) {
    ...
    if (typeof this._htOption.icon !== "undefined") {
      //这里是异步的函数，只有加了await才会变成同步（注意：即使函数里面有await，这个函数也是异步执行的）
      await this._oDrawing.addIcon(this._htOption.icon);
    }
